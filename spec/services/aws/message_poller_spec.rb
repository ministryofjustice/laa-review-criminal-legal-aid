...
RSpec.describe Aws::MessagePoller do
  ...
  let(:message) do
    Aws::SQS::Types::Message.new(
      ...
      body:
        ...
        '\"data\":' \
        '{' \
        '\"id\":\"3201a700-921c-4e8e-93b0-df9ff5fe7328\",' \
        ...
        '"MessageAttributes": {"event_name": {"Type": "String", "Value": "apply.submission"}},' \
        '"SignatureVersion": "1",' \
  ...
  describe '#start!' do
    ...
    it 'creates an ApplicationReceived event' do
      expect { message_poller.start! }.to change { review.state }.from(nil).to(:open)
                                                                 .and not_change {
                                                                        review.parent_id
                                                                      }.from(nil)
                                                                       .and change {
                                                                              review.work_stream
                                                                            }.from(nil).to('criminal_applications_team')
    end
